  ...
  (function(){
    ...
    function typeWord(word, cb){
      ...
      (function step(){
        if(!active) return;
        typeEl.textContent = word.slice(0, i + 1);
        i++;
        if(i < word.length){ setTimeout(step, 90); }
        else{ setTimeout(cb, 550); }
      })();
    }
    ...
    function hideLoader(){
      ...
      hidden = true;
      active = false;
      if(barFill) barFill.classList.add('loader-bar-done');
  ...
  const navLinks = [...document.querySelectorAll('nav.pill a, .mobile-tabbar a')];
  ...
  // Mobile bottom-bar theme tab — mirrors the desktop switch, sharing the
  // same setTheme() so both stay in sync regardless of which is visible.
  // It's also draggable: a press-and-drag repositions it anywhere on
  // screen (clamped to the viewport), while a plain tap still toggles
  // the theme. Position is remembered across visits via localStorage.
  const themeBtnMobile = document.getElementById('themeToggleMobile');
  if (themeBtnMobile) {
    const FAB_POS_KEY = 'themeFabPos';
    const DRAG_THRESHOLD = 6; // px of movement before a press counts as a drag, not a tap

    function clamp(val, min, max){ return Math.min(Math.max(val, min), max); }

    function placeFab(left, top){
      const w = themeBtnMobile.offsetWidth || 42;
      const h = themeBtnMobile.offsetHeight || 42;
      const maxLeft = window.innerWidth - w - 6;
      const maxTop = window.innerHeight - h - 6;
      const x = clamp(left, 6, Math.max(6, maxLeft));
      const y = clamp(top, 6, Math.max(6, maxTop));
      themeBtnMobile.style.left = x + 'px';
      themeBtnMobile.style.top = y + 'px';
      themeBtnMobile.style.right = 'auto';
      return { x, y };
    }

    // Restore a saved position (if any) once the fab has real dimensions.
    // Transition is suppressed for this initial placement so it appears
    // there directly on load instead of visibly gliding in from the corner.
    (function restoreFabPos(){
      ...
      try {
        ...
        if (typeof x === 'number' && typeof y === 'number') {
          themeBtnMobile.style.transition = 'none';
          placeFab(x, y);
          requestAnimationFrame(() => { themeBtnMobile.style.transition = ''; });
        }
    ...
    themeBtnMobile.addEventListener('pointerdown', (e) => {
      ...
      startLeft = rect.left;
      startTop = rect.top;
      themeBtnMobile.setPointerCapture(e.pointerId);
    });

    themeBtnMobile.addEventListener('pointermove', (e) => {
      if (startX === 0 && startY === 0 && !dragging) return;
      if (!themeBtnMobile.hasPointerCapture || !themeBtnMobile.hasPointerCapture(e.pointerId)) return;
      const dx = e.clientX - startX;
      const dy = e.clientY - startY;
      if (!dragging && Math.hypot(dx, dy) > DRAG_THRESHOLD) {
        dragging = true;
        themeBtnMobile.classList.add('dragging');
      }
      if (dragging) {
    ...
    function endDrag(e){
      if (!themeBtnMobile.hasPointerCapture || !themeBtnMobile.hasPointerCapture(e.pointerId)) return;
      themeBtnMobile.releasePointerCapture(e.pointerId);
      if (dragging) {
        // Removing .dragging first re-enables the left/top transition
        // defined in CSS, so the snap below glides instead of jumping.
        themeBtnMobile.classList.remove('dragging');
        const rect = themeBtnMobile.getBoundingClientRect();
        const centerX = rect.left + rect.width / 2;
        const snapLeft = centerX < window.innerWidth / 2;
        const targetX = snapLeft ? 6 : window.innerWidth - rect.width - 6;
        const { x, y } = placeFab(targetX, rect.top);
    ...
    themeBtnMobile.addEventListener('pointerup', endDrag);
    themeBtnMobile.addEventListener('pointercancel', endDrag);

    // Keep the fab on-screen if the viewport is resized/rotated.
    window.addEventListener('resize', () => {
      if (themeBtnMobile.style.left) {
        placeFab(parseFloat(themeBtnMobile.style.left), parseFloat(themeBtnMobile.style.top));
      }
    });
  ...
  // ---------- Designs grid: masonry packing (Pinterest-style) ----------
  // A plain CSS grid row is only as tall as its tallest cell — that either
  // stretches short cards into blank space, or (with align-items:start)
  // leaves a gap below them before the next row starts. To pack things
  // tightly while still keeping "See all" purely additive (new items just
  // extend the grid — they never reshuffle cards already on screen), each
  // .media-item gets an explicit row-span computed from its own rendered
  // height against the small base row unit set on .media-grid
  // (grid-auto-rows:8px). Recomputed whenever an image finishes loading,
  // the window resizes, or the grid expands/collapses.
  (function(){
    const grid = document.getElementById('designsGrid');
    if(!grid) return;

    const ROW_UNIT = 8; // must match .media-grid's grid-auto-rows
    ...
    function layoutAll(){
      grid.querySelectorAll('.media-item').forEach(layoutItem);
    }

    grid.querySelectorAll('.media-thumb img').forEach(img => {
      const item = img.closest('.media-item');
      if(img.complete && img.naturalWidth){
        layoutItem(item);
      } else {
        img.addEventListener('load', () => layoutItem(item), { once:true });
        img.addEventListener('error', () => layoutItem(item), { once:true });
      }
    });
    ...
    const seeAllBtn = document.getElementById('seeAllDesigns');
    if(seeAllBtn){
      // Re-measure after the reveal/collapse fade + stagger has settled.
      seeAllBtn.addEventListener('click', () => setTimeout(layoutAll, 550));
    }
  })();

  // ---------- Projects: "See all" reveal + collapse (Websites & Designs) ----------
  // The always-visible cards live first in the DOM and the "extra" ones are
  // appended after them (see index.html), so expanding/collapsing only ever
  // adds or removes content below the first line — the first row never
  // shifts position. On top of that, extras fade + slide in with a light
  // stagger instead of popping in instantly, and there's now a matching
  // "Show less" to collapse back down.
  (function(){
    const STAGGER_MS = 45;
    const TRANSITION_MS = 450;

    function wireSeeMore(gridId, btnId, itemSelector){
      const grid = document.getElementById(gridId);
      const btn = document.getElementById(btnId);
      if(!grid || !btn) return;

      const label = btn.querySelector('.see-more-label');
      const moreText = btn.dataset.labelMore || (label ? label.textContent.trim() : btn.textContent.trim());
      const lessText = btn.dataset.labelLess || 'Show less';
      ...
      // Start every extra item in the "faded" state so the first reveal animates in.
      items().forEach(item => item.classList.add('is-fading'));
      ...
      btn.addEventListener('click', () => {
        ...
        if(!isExpanded){
          // Reveal: un-collapse the grid, then fade+slide each extra item in with a stagger.
          animating = true;
          grid.classList.remove('is-collapsed');
          void grid.offsetWidth; // force reflow so the fade-in actually transitions
          list.forEach((item, i) => {
            setTimeout(() => item.classList.remove('is-fading'), i * STAGGER_MS);
          });
          ...
          setTimeout(() => { animating = false; }, list.length * STAGGER_MS + TRANSITION_MS);
        } else {
          // Collapse: fade the extras out first, then hide the grid and reset the button.
          animating = true;
          list.forEach(item => item.classList.add('is-fading'));
          ...
          setTimeout(() => {
            ...
            // If collapsing left the viewport scrolled past this section's
            // heading, ease back up to it instead of stranding the user
            // over a now-empty gap.
    ...
    wireSeeMore('websitesGrid', 'seeAllWebsites', '.extra-project');
    wireSeeMore('designsGrid', 'seeAllDesigns', '.extra-design');
    wireSeeMore('caseStudiesGrid', 'seeAllCaseStudies', '.extra-project');
  })();
  ...
  // ---------- swipeable tools marquee (keeps auto-scroll) ----------
  (function initToolsMarquee(){
    ...
    const LOOP_MS = 32000;   // matches the original 32s CSS animation duration

    let setWidth = 0;   // px width of one tool-set (track holds 2 duplicate sets)
    let pxPerMs = 0;
    let offset = 0;     // current translateX in px (0 or negative)
    let lastTs = null;
    let dragging = false;
    let dragStartX = 0;
    ...
    function measure(){
      setWidth = track.scrollWidth / 2 || 1;
      pxPerMs = setWidth / LOOP_MS;
    }
    ...
    function tick(ts){
      ...
      const dt = ts - lastTs;
      lastTs = ts;
      if(!dragging && !hovering && !userPaused){
        offset -= pxPerMs * dt;
        normalize();
        render();
      }
    ...
    // switch from the CSS keyframe animation to JS-driven transform
